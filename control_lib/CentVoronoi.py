...
class CentVoronoi():
    def __init__(self, pos, roi_vert=None, sensingRadius = None, grid_size = 0.05):
        ...
        # Define default circle to optimize calculation of center mass. 
        theta = np.linspace(0, np.pi*2, 100) # divide circle into 100 vertices
        self._circle_linspace = np.transpose(np.array([np.cos(theta), np.sin(theta)]))
    ...
    # WARNING: These evaluation of grid are really specific to scenario with AgriControl and AgriJournal
    # Consider to tidy this part in the future (to make it more lean and general purpose)
    def set_points_of_importance(self, points, covhole_width = None):
        ...
        # Identify grids that is considered coverage hole based on the given points
        self._poi_covhole_grid_idx =  np.full(self._grid_num, False)
        if covhole_width is not None: # The value is being set
            for i in range(len(points)):
                is_within_covhole_i = (self._grid_points[:,0] >= (points[i,0] - 0.5*covhole_width)) \
                                    & (self._grid_points[:,0] <= (points[i,0] + 0.5*covhole_width)) \
                                    & (self._grid_points[:,1] >= (points[i,1] - 0.5*covhole_width)) \
                                    & (self._grid_points[:,1] <= (points[i,1] + 0.5*covhole_width))
                self._poi_covhole_grid_idx |= is_within_covhole_i
    ...
    # another version of set_density_from_points: SUM of gaussian with addtional floor
    def set_custom_density_with_floor(self, points, sigma=[[0.25, 0], [0, 0.25]], gaussian_ratio=0.5, floor=0.1):
        # Generate density function
        temp = np.zeros(self._grid_densVal.shape)
        for p in points:
            rv = multivariate_normal([p[0], p[1]], sigma)
            temp += rv.pdf(self._grid_points) # defaulting to sum of pdf
        # It should cancel the original gain of 1 / [ 2 pi sqrt(det(Sigma)) ]
        # normalize gaussian peak to 0 and 1 (not neccessarily max value)
        beta = 2. * np.pi * np.sqrt(np.linalg.det(sigma)) 
        temp *= gaussian_ratio*beta # set gaussian peak to gaussian_ratio 
        temp += floor # add floor
        # ensure still below 1
        if temp.any() > 1.0: assert False, f"There exist density function greater than 1: {temp}. \
            Fix this by adjusting the sigma, gaussian_ratio, and floor"
    ...
    def _compute_center_mass(self, vertices, i, use_sensing_rad_tocompute = True):
        if use_sensing_rad_tocompute:
            # Only compute points within sensing region
            dist = np.linalg.norm(self._grid_points[:,:2] - self._pos[i][:2], axis=1)
            is_within_sensing = dist < self._sens_rad[i] + self._grid_size # give some margin
            # is_contained = dist < self._sens_rad[i] - self._grid_size

            # Calculate center of mass. Here we only shift the default circle. 
            sensR_xy = np.array([self._pos[i][:2]]) + (self._circle_linspace*self._sens_rad[i])
            # intersect ROI with sensing radius
            mass_dist = geom.Polygon(vertices).intersection(geom.Polygon(sensR_xy)) 

        else: # use the whole ROI
            is_within_sensing = [True]*self._grid_num # check all points
            mass_dist = geom.Polygon(vertices)
        
        R = np.zeros(2) 
        M = 0
        for j in range(self._grid_num): 
            if is_within_sensing[j]:
                m = self._grid_densVal[j]
                if not mass_dist.contains(self._grid_polygon[j]):
                    ratio_grid_poly = (self._grid_polygon[j].intersection(mass_dist).area / self._grid_polygon[j].area)                
                    m *= ratio_grid_poly
    ...
    def compute_nominal_control(self, use_sensing_rad_tocompute=True, scale_nom = 1.): # TODO: change the name
        # set default values
        ...
        for i in range(len(self._pos)):
            if (self._vor_obj is not None) and (self._vor_vertices[i] is not None): # if points is within ROI
                centroids[i,:], mass, self._sensing_region[i] = self._compute_center_mass(self._vor_vertices[i], i, use_sensing_rad_tocompute)
                gain = 5*mass*scale_nom
                distC = centroids[i] - self._pos[i]                

            else: # Outside of ROI: set limited gain based on distance so it is moving within sensing radius
                distC = centroids[i] - self._pos[i]
    ...
    def extract_covhole_density_data(self): 
        # Special function to show the density value within a box around the specified point 
        # that was used to create the original density function
        return self._grid_points[self._poi_covhole_grid_idx], self._grid_densVal[self._poi_covhole_grid_idx]
...
class CentVoronoiTVDensity(CentVoronoi):
    ...
    def __init__(self, pos, roi_vert=None, sensingRadius = None, grid_size = 0.05):
        ...
        # Metric to evaluate coverage
        self._totalCoverageRatio = 1.

    def get_total_coverage_ratio(self): 
        return self._totalCoverageRatio, self._sum_grid_densVal, self._sum_default_densVal
    def get_covhole_coverage_ratio(self): 
        return self._covHole_CoverageRatio, self._covHole_grid_densVal, self._covHole_default_densVal
    ...
    def _update_density_function(self, dt): 
        ...
        if self._roi_vert is not None:
            ...
            if sensed_idx is not None: # Safety check: None happen when there is no agent assigned
            ...
            # Update Metric to evaluate TOTAL coverage performace
            self._sum_default_densVal = self._default_densVal.sum()
            self._sum_grid_densVal = self._grid_densVal.sum()
            self._totalCoverageRatio = ( self._sum_grid_densVal / self._sum_default_densVal )
            # Update Metric to evaluate coverage performace on COVERAGE HOLE ONLY
            self._covHole_default_densVal = self._default_densVal[self._poi_covhole_grid_idx].sum()
            self._covHole_grid_densVal = self._grid_densVal[self._poi_covhole_grid_idx].sum()
            self._covHole_CoverageRatio = ( self._covHole_grid_densVal / self._covHole_default_densVal )
